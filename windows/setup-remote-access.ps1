...
if (Test-Path $claudeExe) {
    ...
    try {
        ...
        Write-Warn "  (Skipping — install after SSH access is established)"
    }
}

# ─── 8. CONFIGURE CLAUDE DESKTOP MCP ─────────────────────────────────────────
...
try {
    ...
    if ($dockerCmd) {
        ...
        Write-Warn "docker not in PATH yet — volume will be created on first run"
    }
} catch {
...
Write-Host @"
  ...
  ║    (Tailscale SSH — key-based, no password)                  ║
  ║                                                              ║
